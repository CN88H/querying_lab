...
def problem_one(request):
    ...
    # Student.objects.filter(student_gpa > 3.0)
    for student in student_gpa:
    # for student in range(student_gpa):

      print(f"First Name: {student.first_name} {student.last_name} GPA: {student.gpa}")
...
def problem_two(request):
    ...
    instructor_hired_date = Instructor.objects.filter(hire_date__year__lt=2010).order_by('hire_date')

    for instructor in instructor_hired_date:
...
def problem_three(request):
    ...
    instructor_id = Instructor.objects.get(pk=2)

    courses = Course.objects.filter(instructor_id=2)

    # print(f'Instructor Name: {instructor_id} \n Courses: {course.}')
...
def problem_four(request):
    ...
    # for student in student_count:
      # print(f'Students Count: {student}')
...
def problem_five(request):
    ...
    # create_new_student = Student.objects.create(pk=11, first_name='Chee', last_name='Her', year=2020, gpa=3)
...
def problem_six(request):
    ...
    # student_id = Student.objects.filter(pk=11).update(gpa=3.5)
    # # for student in student_id:
    updated_student = Student.objects.get(id=11)
...
def problem_seven(request):
    ...
    # Make sure to set this equal to the primary key of the row you just created!
    student_id = Student.objects.delete(id__gte=12)
